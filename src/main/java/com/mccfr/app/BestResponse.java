...
//                }
            ...
            for (int j = 0; j < NUM_PLAYERS; j++) {
                ...
                updatePlayer = j;
                float out = walkTree(Leduc.STARTING_HISTORY, gameState, 1);
    ...
    private float walkTree(String history, GameState gameState, float reach){
        if(game.isTerminal(history, gameState)){
            ...
            return walkTree(nextHistory, newGameState, reach);
        }
...
//        cfrThread.actionNames.put(key, actionNames);
        ...
        if (updatePlayer == gameState.currentPlayer) {
            ...
            for (int a = 0; a < actions.length; a++) {
                ...
                actionUtils[a] = walkTree(nextHistories[a], newGameState, reach);
            ...
            nodeUtil = walkTree(nextHistories[action_idx], newGameState, reach);
        }
        return nodeUtil;
    }
}
